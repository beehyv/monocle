...
after ``pip install monocle_test_tools[models]``), so that model
...
def download_all() -> None:
    ...
    Skips models that are already cached.  If the optional ``[models]``
    dependencies are not installed, raises ``ImportError``.
    """
